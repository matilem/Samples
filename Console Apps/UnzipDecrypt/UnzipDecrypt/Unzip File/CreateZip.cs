...
namespace UnzipDecrypt.ZipFile
{
    ...
    {
        public static void BuildZip(string[] zipdata)
        {
            ...
            if (!Directory.Exists(zipdata[0]))
            {
                Console.WriteLine("Cannot find directory '{0}'", zipdata[0]);
                return;
            }
            ...
            try
            {
                ...
                string[] filenames = Directory.GetFiles(zipdata[0]);

                // 'using' statements guarantee the stream is closed properly which is a big source
                // of problems otherwise.  Its exception safe as well which is great.
                using (ZipOutputStream s = new ZipOutputStream(File.Create(zipdata[1])))
                {
