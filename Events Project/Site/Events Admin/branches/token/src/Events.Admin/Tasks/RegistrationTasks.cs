...
namespace Aafp.Events.Admin.Tasks
{
    public class RegistrationTasks : IRegistrationTasks
    {
        ...
        public CustomerSearchViewModel Search(UserSearchViewModel model)
        {
            ...
            try
            {
                ...
                foreach (var item in viewModel.Results)
                {
                    ...
                    foreach (var recentItem in item.RecentEvents)
                    {
                        if (recentItem.RelatedRegistrationKeys.Any())
                        {
                            foreach (var key in recentItem.RelatedRegistrationKeys)
                            {
                                recentItem.RelatedRegistrations.Add(item.RecentEvents.FirstOrDefault(x => x.RegistrationKey == key));
                                itemsToRemove.Add(key);
                            }
                        }
                    }

                    foreach (var pastItem in item.PastEvents)
                    {
                        if (pastItem.RelatedRegistrationKeys.Any())
                        {
                            foreach (var key in pastItem.RelatedRegistrationKeys)
                            {
                                pastItem.RelatedRegistrations.Add(item.RecentEvents.FirstOrDefault(x => x.RegistrationKey == key));
                                itemsToRemove.Add(key);
                            }
                        }
                    }

                    item.RecentEvents.RemoveAll(x => itemsToRemove.Contains(x.RegistrationKey));
                    item.PastEvents.RemoveAll(x => itemsToRemove.Contains(x.RegistrationKey));
                }
        ...
        public RegistrationTypeViewModel GetEventRegistrationTypeInfoByCustomer(Guid eventKey, Guid customerKey,
        ...
        public RegistrationViewModel GetNewRegistration(Guid eventKey, Guid customerKey,
        ...
        public AafpServiceResult<Guid> UpdatePendingRegistration(Guid eventKey, Guid customerKey,
            Guid registrationTypeKey, DateTime registrationDate, RegistrationViewModel registration)
        {
            registration.PriceKey = registrationTypeKey;
            registration.RegistrationDate = registrationDate;

            return SavePendingRegistration(registration);
        }

        public AafpServiceResult<Guid> SavePendingRegistration(RegistrationViewModel model)
        {
            AafpServiceResult<Guid> result = null;

            try
            {
                result = HttpClientHelper.PostJson<Guid>(eventService, "admin/registration/save-new-registration", model);
            ...
            {
                result.ErrorMessage = "We are unable to save the registration at this time.";
            }

            return result;
        }

        public AafpServiceResult<Guid> SaveEditRegistration(EditRegistrationViewModel model)
        {
            AafpServiceResult<Guid> result = null;

            try
            {
                result = HttpClientHelper.PostJson<Guid>(eventService, "admin/registration/save-edit-registration",
                    model);
        ...
        public EditRegistrationViewModel GetEditRegistrationViewModel(Guid registrationKey)
        {
            var viewModel = new EditRegistrationViewModel();
        ...
        private void SetUpSessionCapacity(List<EditStepViewModel> steps)
        {
            foreach (var step in steps)
            {
                foreach (var heading in step.Headings)
                {
                    foreach (var session in heading.Sessions)
                    {
                        session.SessionCapacity = new SessionCapacityViewModel
                        {
                            ...
                            SessionTitle = session.Title,
                            SessionCapacity = session.Capacity,
                            SessionRegistrantCount = session.RegistrantCount,
                            SessionTicketed = session.Ticketed
                        };
                    }
                }
