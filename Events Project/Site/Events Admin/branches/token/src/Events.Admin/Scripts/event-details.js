$(document).ready(function () {
    ...
    $("#addNewEventButton").click(function (event) {
        event.preventDefault();
        $("#addNewEventButton").hide();
        $("#saveNewEvent").show();
    });
    $("#addNewEventButton").on("click", function (event) {
        event.preventDefault();
        ...
        $.get("get-registrant-details/event/" + eventKey + "/customer/" + customerKey, function (data) {
            if (data.Key !== "00000000-0000-0000-0000-000000000000") {
                ...
                if (registrationTypeKey == undefined || registrationDate === "" || eventKey === "") {
                    $("#addNewEventError").html('<div class="alert-box -warning"><div class="alert-box__icon"><span class="fa fa-exclamation-circle fa-2x -white"></span></div><div class="alert-box__content">Please select a registration date and type.</div></div>');
                }
                else {
                    window.location.href = "new?eventKey=" + eventKey + "&customerKey=" + customerKey + "&registrationTypeKey=" + registrationTypeKey + "&registrationDate=" + registrationDate;
                }
            }
        }
);
